    ...
    <v-row>
      ...
      <v-col cols="12">
        <v-text-field v-model="newItem" label="新增事項" append-icon="mdi-plus" @click:append="onInputSubmit"
          @keydown.enter="onInputSubmit" :rules="[rules.required, rules.length]" ref="newItenInput"></v-text-field>
        <v-table>
          <thead>
            <tr>
              <th>名稱</th>
          ...
          <tbody>
            <tr v-for="(item, i) in items" :key="item.id">
              <td>
                <v-text-field v-model="item.model" :rules="[rules.required, rules.length]" autofocus ref="editItemInput"
                  @keydown.enter="onEditSubmit(item.id, i)" v-show="item.edit"></v-text-field>
              ...
              <td>
                <template v-if="item.edit">
                  <v-btn icon="mdi-check" color="green" variant="text" @click="onEditSubmit(item.id, i)"></v-btn>
                  <v-btn icon="mdi-undo" color="red" variant="text" @click="cancelEditItem(item.id)"></v-btn>
                </template>
                <template v-else>
                  <v-btn icon="mdi-pen" color="green" variant="text" @click="editItem(item.id)"></v-btn>
...
const onInputSubmit = async () => {
  const validate = await newItenInput.value.validate()
  console.log(validate)
  if (validate.length > 0) return
  addItem(newItem.value)
  newItenInput.value.blur()
  newItenInput.value.reset()
  // newItem.value = ''
  // 把v - model清空
}

const onEditSubmit = async (id, i) => {
